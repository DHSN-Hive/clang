...
{
	if(firstElement == NULL) InsertElement(n, na);
	else
	{
		sname *nE = NewElement(n, na);
		lastElement->next = nE;
 ...
 * 	pE aE nE nnE
 *
 * Es müssen pE->next, aE->prev, aE->next, nE->prev, nE->next, nnE->prev angepasst werden	
 */
sname *Swap(sname *aE)
{
	if(aE == NULL || aE->next == NULL) return aE;
...
void Sort()
{
	...
	if(aE == NULL || aE->next == NULL) return;
