...
struct student {
  ...
  char *nachname;
  int mtr;
} staticstudent, *dynamicstudent; /*staticstudent & dynamicstudent sind globale
                                     Variablen vom Typ student*/
...
int main(void) {
  ...
  /*Ausgabe*/
  printf("%s %s %d", staticstudent.vorname, staticstudent.nachname,
         staticstudent.mtr);
  ...
  /*Ausgabe*/
  printf("%s %s %d", dynamicstudent->vorname, dynamicstudent->nachname,
         dynamicstudent->mtr);
 ...
 *	im dynamischen Bereich (Heap) 8Byte Pointer Vorname + 8Byte Pointer
 * Vorname + 4Byte int mtr + 2*32Byte für malloc Vor- und Nachname
 *
 *
 *
 *
