...
{
	/*Funkltion hängt an Liste an und gibt die Adresse des letzten Elements zurück*/
...
{
	/*Funktion hängt ein Element vorn an die Liste und gibt das neue erste Element zurück*/

	if(fE == NULL) return AddElement(fE, n, na);
	else
	{
		sname *newElement = NewElement(n, na);
		newElement->next = fE;
...
sname *sort(sname *fE)
{
	...
	while(tE->next != NULL)
	{
		if(tE->nr > tE->next->nr)
		{
			if(pE == NULL) fE = tE->next;
			swap(pE, tE); /*tE und tE->next werden getauscht, pE wird mitgegeben, weil es geändert werden muss*/
			tE = fE;
			pE = NULL;
		}
		else
...
int main(void)
{
	...
	firstElement = lastElement = InsertElement(firstElement, 12, "Hans Mustermann");
